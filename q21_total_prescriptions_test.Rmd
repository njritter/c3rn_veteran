...
r <- geom_histogram(q_data(aes(x = q_mean_data, y = value(response))))
r
```

```{r}
r <- ggplot(q_data, aes(x=response)) + 
  geom_histogram(stat="count") +
  labs(title="Total prescriptions", x="# of meds", y="Respondent count")
r
```

```{r}
ggsave(filename = 'q21_total_prescriptions.png', plot = r, device = 'png')
write.csv(q_plot, 'q21_total_prescriptions_144.csv', row.names = FALSE)
...
```{r}
ggsave(filename = 'q21_total_prescriptions.png', plot = r, device = 'png')
write.csv(q_plot, 'q21_total_prescriptions_333.csv', row.names = FALSE)
```
