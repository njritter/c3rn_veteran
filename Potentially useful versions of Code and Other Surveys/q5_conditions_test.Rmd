...
colnames(test_data) <- test_data[1, ]
  ...
  test_data <- test_data %>% rename(id = 1)
```

```{r}
q_n <- length(unique(test_data$id))
q_plot <- clean_choose_all(test_data)
...
write.csv(q_plot, 'output/q5_conditions_144.csv', row.names = FALSE)
...
```{r}
ggsave(filename = 'output/q5_condition.png', plot = r, device = 'png')
write.csv(q_plot, 'output/q5_condition_232.csv', row.names = FALSE)
write.csv(qf_all_data, 'output/q5_condition_ids.csv', row.names = FALSE)
```
